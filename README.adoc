...
Par défaut, seules les règles typographiques hors espaces insécables sont appliquées
(unités, ordinaux, espaces superflues…). Pour ajouter les espaces insécables :

[source,bash]
----
french-typo --nbsp mon_fichier.adoc
----
...
* 🔤 en ignorant les marqueurs structurels (titres `= …`, attributs `:clé: valeur`,
  chemins relatifs `./…`) et les URL, jamais touchés par les règles typographiques ;
* 🈳 en écrivant le vrai caractère insécable Unicode (`U+00A0`), pas l’entité HTML
  `&nbsp;` — celle-ci reste réservée à l’adaptateur Anki, pensé pour du HTML. Un fichier
  déjà formaté par une version antérieure de l’outil (avec l’entité) est relu et
